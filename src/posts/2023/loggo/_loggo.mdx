...
It’s 18:00 on a Friday and I’m prepping for the weekend. The proverbial foot outside the office and into a bar or rather a museum. On the other side of the Atlantic, New York, the city that never sleeps, is waking up.
...
I recall my foot into the office and read the report. Surprisingly, I don’t feel nervous. I suppose that when the stakes reach preposterous levels, they become abstract in one’s mind.
...
![The UX screams at you](order.png)

But is the feature enabled? A glance at the logs results in a relieving yes. So I reply with my working theory, the customer acknowledges that this is indeed the case, closes the incident, and wishes a great weekend. Likewise!

Then, with an anvil off my chest, I leave the office, meet with friends, and offer them wine or rather valuable insights on the painting scene in the early Renaissance.

![Bacchus](renaissance.png)
...
![The UX screams at you](shout.jpeg)
...
I speak out of experience when I say that you don’t want to receive the following Slack message while you are watching Barbie or rather Oppenheimer…
...
I would have written shorter logs, but I did not have the time.

Since you are going to spend 80% of your time reading 20% of the logs, wouldn’t you be better off pruning the remaining 80% of the logs so that you end up spending 100% of the time on… wait this is really confusing.
...
![The UX screams at you](read.png)
...
The people responsible for this should be in prison, although with a light sentence.

![The UX screams at you](prison.png)
...
I should now suggest to use emojis in your ToStrings, but there is a whole paragraph on that…
5 - Use emojis
...
![The UX screams at you](emojis.png)
...
As an example, this is one of the applications we have at Go Autonomous...

![The UX screams at you](logs.png)

n this snippet, we are notified of a new email from a customer inbox 📮 via Microsoft Graph Api, we enqueue it 🥱 for processing 🚀, validate it🚦 and feed it to our powerful AI 🤖. Then, we tag 🏷 the email, perform digital transformations 🚚 on the extracted data, and verify that the products 🛒 exist in the catalog 🏆. If all is kosher, we act 🎭 upon the information we gathered by calling our customers’ ERP systems, and if everything is executed with no user intervention, we get a well-deserved 💙. Finally, we save the data via a symbol that the younger generation will never fully understand 💾.
(But why are we using a truck 🚚 to represent digital transformations? No reason.)

![The UX screams at you](optimus.png)
...
![The UX screams at you](dumbledore.png)

All thanks to the strategies described above.
And when they ask me how did I find the bug so quickly, I can reply with pride:
It is the poop 💩 emoji, Harry, that shows where the problem is.
